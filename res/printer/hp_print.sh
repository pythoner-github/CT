#!/bin/sh
# 脚本使用格式
# hp_print.sh Tool_path Print_device PPD_file Image_name [Copies] [Options] 
...
then
    ...
    echo "use : hp_print.sh Tool_path Print_device PPD_file Image_name [Copies] [Options]"
    echo "exit"
    exit
fi

if [ ! -e "$1/imagetoraster" -o ! -e "$1/rastertopclx" ]
then 
...
$TOOLPATH/imagetoraster 1 Emperor image $COPIES "$OPTIONS" $4 | $TOOLPATH/rastertopclx 1 Emperor image 1 NULL | cat > $2
